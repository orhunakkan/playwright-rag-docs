...
> **Source:** [developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Temporal/PlainDate/era](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Temporal/PlainDate/era)
...
- The following calendars have a single era:
  ...
  - `indian`: `"shaka"`
  - `persian`: `"ap"`
- The following calendars have two eras. One is the _epoch era_, in which `eraYear` starts at 1 and is the same as `year`. The other is the inverse era, in which `eraYear` also starts at 1 and is equal to `1 - year` (so `eraYear: 1` corresponds to year `0`, `eraYear: 2` to year `-1`, etc.):
