...
> **Source:** [developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/String/substring](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/String/substring)
...
`length` property to extract the last characters of a
...
`substr()` methods, so you should be careful not to get
them confused.
...
The `substring()` and `slice()` methods are
...
See the `slice()` page for more examples with negative
numbers.
