...
> **Source:** [developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Trailing_commas](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Trailing_commas)
...
If more than one trailing comma is used, an elision (or hole) is produced. An array with holes is called [_sparse_](/en-US/docs/Web/JavaScript/Guide/Indexed_collections#sparse_arrays) (a _dense_ array has no holes). When iterating arrays for example with `Array.prototype.forEach()` or `Array.prototype.map()`, array holes are skipped. Sparse arrays are generally unfavorable, so you should avoid having multiple trailing commas.
...
Function parameter definitions or function invocations only containing a comma will throw a `SyntaxError`. Furthermore, when using [rest parameters](/en-US/docs/Web/JavaScript/Reference/Functions/rest_parameters), trailing commas are not allowed:
