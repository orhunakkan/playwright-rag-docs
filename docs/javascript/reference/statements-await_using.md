...
> **Source:** [developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Statements/await_using](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Statements/await_using)

---

The **`await using`** declaration declares block-scoped local variables that are _asynchronously disposed_. Like `const`, variables declared with `await using` must be initialized and cannot be reassigned. The variable's value must be either `null`, `undefined`, or an object with a [`[Symbol.asyncDispose]()`](/en-US/docs/Web/JavaScript/Reference/Global_Objects/Symbol/asyncDispose) or [`[Symbol.dispose]()`](/en-US/docs/Web/JavaScript/Reference/Global_Objects/Symbol/dispose) method. When the variable goes out of scope, the `[Symbol.asyncDispose]()` or `[Symbol.dispose]()` method of the object is called and awaited, to ensure that resources are freed.
