...
> **Source:** [developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Date](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Date)
...
- `toISOString()` returns a string in the format `1970-01-01T00:00:00.000Z` (the date time string format introduced above, which is simplified [ISO 8601](https://en.wikipedia.org/wiki/ISO_8601)). `toJSON()` calls `toISOString()` and returns the result.
...
- `toUTCString()` returns a string in the format `Thu, 01 Jan 1970 00:00:00 GMT` (generalized ).
- `toLocaleDateString()`, `toLocaleTimeString()`, and `toLocaleString()` use locale-specific date and time formats, usually provided by the [`Intl`](/en-US/docs/Web/JavaScript/Reference/Global_Objects/Intl) API.
...
- `Date.prototype.toTemporalInstant()`
  - : Returns a new `Temporal.Instant` object with the same `epochMilliseconds` value as this date's [timestamp](#the_epoch_timestamps_and_invalid_date).
...
In this case, it's important to return only an integer—so a simple division won't do. It's also important to only return actually elapsed seconds. (That's why this code uses `Math.floor()`, and _not_ `Math.round()`.)
