...
> **Source:** [www.typescriptlang.org/docs/handbook/namespaces.html](https://www.typescriptlang.org/docs/handbook/namespaces.html)
...
for (let s of strings) {
  for (let name in validators) {
    let isMatch = validatorsname.isAcceptable(s);
...
for (let s of strings) {
  for (let name in validators) {
    console.log(
      `"${s}" - ${
        validatorsname.isAcceptable(s) ? "matches" : "does not match"
      } ${name}`
    );
  }
}
