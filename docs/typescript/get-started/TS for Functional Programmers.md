...
> **Source:** [www.typescriptlang.org/docs/handbook/typescript-in-5-minutes-func.html](https://www.typescriptlang.org/docs/handbook/typescript-in-5-minutes-func.html)
...
```ts
anys.map(anys1); // oh no, "oh no" is not a function
```
...
```ts
let sepsis = anys0 + anys1; // this could mean anything
```
