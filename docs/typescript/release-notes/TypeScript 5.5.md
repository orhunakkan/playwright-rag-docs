...
> **Source:** [www.typescriptlang.org/docs/handbook/release-notes/typescript-5-5.html](https://www.typescriptlang.org/docs/handbook/release-notes/typescript-5-5.html)
...
In the above, neither `obj` nor `key` are ever mutated, so TypeScript can narrow the type of `objkey` to `string` after the `typeof` check.
