...
> **Source:** [playwright.dev/python/docs/other-locators](https://playwright.dev/python/docs/other-locators)
...
page.locator("//spancontains(@class, 'spinner__loading')|//div@id='confirmation'").wait_for()
```
...
```py
# Waits for either confirmation dialog or load spinner.
await page.locator("//spancontains(@class, 'spinner__loading')|//div@id='confirmation'").wait_for()
```
...
```txt
css=article >> css=.bar > .baz >> css=spanattr=value
```
