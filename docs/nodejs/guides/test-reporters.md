...
| `PLAYWRIGHT_FORCE_TTY` | | Whether to produce output suitable for a live terminal. Supports `true`, `1`, `false`, `0`, `WIDTH`, and `WIDTHxHEIGHT`. `WIDTH` and `WIDTHxHEIGHT` specifies the TTY dimensions. | `true` when terminal is in TTY mode, `false` otherwise.
