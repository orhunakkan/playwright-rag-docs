...
- New method [frameLocator.owner()](/api/class-framelocator.mdx#frame-locator-owner) converts a FrameLocator object to a Locator. This can be useful when you have a FrameLocator object obtained somewhere, and later on would like to interact with the `iframe` element.
  ...
  ```js
  const frameLocator = page.frameLocator('iframename="embedded"');
  // ...
...
```js
await page.locator('_react=SubmitButtonenabled=true').click();
await page.locator('_vue=submit-buttonenabled=true').click();
```
