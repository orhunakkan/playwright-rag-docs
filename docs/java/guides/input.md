...
> **Source:** [playwright.dev/java/docs/input](https://playwright.dev/java/docs/input)
...
Using [Locator.setChecked()](/api/class-locator.mdx#locator-set-checked) is the easiest way to check and uncheck a checkbox or a radio button. This method can be used with `inputtype=checkbox`, `inputtype=radio` and `role=checkbox` elements.
...
page.getByLabel("Upload file").setInputFiles(new Path0);
